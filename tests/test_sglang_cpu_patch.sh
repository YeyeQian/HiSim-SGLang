...
grep -F 'git apply --check --no-index --directory=sglang /tmp/sglang-cpu.patch' "${root_dir}/Dockerfile" >/dev/null ||
  fail 'Dockerfile does not fail on patch drift before applying it'
grep -F 'git apply --no-index --directory=sglang /tmp/sglang-cpu.patch' "${root_dir}/Dockerfile" >/dev/null ||
